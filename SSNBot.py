...
async def time(ctx):
    ...
    if(timeL == "0420"):
        await bot.send("It's 4:20 EDT :clock4: Wooo!  <:blunt:447646223587999765>  <:weed:447646187214864385>")
    elif("0400" <= timeL <= "0429"):
        await bot.send(localtime + " EDT :clock4:")
    elif("0430" <= timeL <= "0459"):
        await bot.send(localtime + " EDT :clock430:")
    elif( "0500" <= timeL <= "0529"):
        await bot.send(localtime + " EDT :clock5:")
    elif("0530" <= timeL <= "0559"):
        await bot.send(localtime + " EDT :clock530:")
    elif("0600" <= timeL <= "0629"):
        await bot.send(localtime + " EDT :clock6:")
    elif("0630" <= timeL <= "0659"):
        await bot.send(localtime + " EDT :clock630:")
    elif("0700" <= timeL <= "0729"):
        await bot.send(localtime + " EDT :clock7:")
    elif("0730" <= timeL <= "0759"):
        await bot.send(localtime + " EDT :clock730:")
    elif("0800" <= timeL <= "0829"):
        await bot.send(localtime + " EDT :clock8:")
    elif("0830" <= timeL <= "0859"):
        await bot.send(localtime + " EDT :clock830:")
    elif("0900" <= timeL <= "0929"):
        await bot.send(localtime + " EDT :clock9:")
    elif("0930" <= timeL <= "0959"):
        await bot.send(localtime + " EDT :clock930:")
    elif("1000" <= timeL <= "1029"):
        await bot.send(localtime + " EDT :clock10:")
    elif("1030" <= timeL <= "1059"):
        await bot.send(localtime + " EDT :clock1030:")
    elif("1100" <= timeL <= "1129"):
        await bot.send(localtime + " EDT :clock11:")
    elif("1130" <= timeL <= "1159"):
        await bot.send(localtime + " EDT :clock1130:")
    elif("1200" <= timeL <= "1229"):
        await bot.send(localtime + " EDT :clock12:")
    elif("1230" <= timeL <= "1259"):
        await bot.send(localtime + " EDT :clock1230:")
    elif("0100" <= timeL <= "0129"):
        await bot.send(localtime + " EDT :clock1:")
    elif("0130" <= timeL <= "0159"):
        await bot.send(localtime + " EDT :clock130:")
    elif("0200" <= timeL <= "0229"):
        await bot.send(localtime + " EDT :clock2:")
    elif("0230" <= timeL <= "0259"):
        await bot.send(localtime + " EDT :clock230:")
    elif("0300" <= timeL <= "0329"):
        await bot.send(localtime + " EDT :clock3:")
    elif("0330" <= timeL <= "0359"):
        await bot.send(localtime + " EDT :clock4:")
    else:
        await bot.send(localtime + " EDT")


@bot.command(pass_context=True)
async def fgt(ctx):
    await bot.send("Honestly, " + (ctx.author.mention) + " I came here to have a good time and you're just making it more shite.")

@bot.command(pass_context=True)
async def fag(ctx, user: discord.Member):
    await bot.send("Honestly, you're being such a fgt. " + (user.mention))
...
async def kick(ctx, user: discord.Member):
    await bot.send(":boot: :ear_of_rice: Not in my, {} ricefields! :ear_of_rice: :boot:".format(user.mention) )
...
async def hug(ctx, user: discord.Member):
    await bot.send("{} gives hugs to {}!".format(ctx.author.mention, user.mention))
...
async def slap(ctx, user: discord.Member):
    ...
    if( "2200PM" <= localtime <= "2359PM"):
        await bot.send("{} tiredly slaps {}!".format(ctx.author.mention, user.mention))
    elif( "0000AM" <= localtime <= "0600AM"):
        await bot.send("{} tiredly slaps {}!".format(ctx.author.mention, user.mention))
    else:
        await bot.send("{} slaps {}!".format(ctx.author.mention, user.mention))
